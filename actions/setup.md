...
The Intrusion feature is an **alias**, not an action — see
[`../aliases/intrusion.alias`](../aliases/intrusion.alias) and the
"Wizard — Occultist" section of the [README](../README.md). The alias creates
and resets its own `Intrusion Step` counter, so there is nothing to set up by
hand: install it, then run `!intrusion setup` once.
